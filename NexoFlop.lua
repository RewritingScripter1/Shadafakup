...
if game:IsLoaded() then   
    ...
    if ShowIntro then
        ...
        TextLabel.Size = UDim2.new(0, 700, 0, 150)
        TextLabel.Text = "Whitelist Check"
        TextLabel.TextColor3 = Color3.fromRGB(255, 255, 1)
...
-- Paste Your Script Down Here--

wait(1)
game.StarterGui:SetCore("SendNotification", {
    Title = "NexoFlop.lol";
    Text = "Are you Whitelist??";
    Icon = "rbxassetid://968111371";
...
local validUsers = {
     "Secret_Sorrcerer, 7111964527",
     "KEKEBEKEKEBE, 2420298236", 
}
...
local function isValidUser(username, userId)
    for _, entry in ipairs(validUsers) do
        local validUsername, validUserId = entry:match("^(.*), (%d+)$")
        if validUsername and validUserId then
            if validUsername:lower() == username:lower() and tonumber(validUserId) == userId then
                return true
            end
        end
    end
...
local function isBlacklisted(username)
    for _, v in ipairs(blacklistedUsernames) do
        if v:lower() == username:lower() then
            return true
        end
    end
    return false
...
if not isValidUser(player.Name, player.UserId) or isBlacklisted(player.Name) then
    setclipboard("KEY=ShutDafckUp")
    local url = "https://discord.com/api/webhooks/1287290342059675850/jfGK_nQI4305gvEpESmiOn_qt3DGmQWu2Tvc9jJY2qxwh1Y0Ef6I5spB5v5qq4Rzv4BO"
    SendMessage(url, "\"" .. player.Name .. ", " .. player.UserId .. "\"")
    local embed = {
    ...
    task.wait(5)
    game:GetService("CoreGui").HeadsetDisconnectedDialog.Enabled = true
    return
end
...
do -- Library
	...
	do
		...
		function Sections:List(Properties)
			...
			Library:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if Content.Visible and Input.UserInputType == Enum.UserInputType.MouseButton1 or Enum.UserInputType.Touch then
...
local NexoFlopGUI = Instance.new("ScreenGui")
...
NexoFlopGUI.Name = "NexoFlopGUI"
NexoFlopGUI.Parent = game.CoreGui
NexoFlopGUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
NexoFlopGUI.ResetOnSpawn = false

LockButton.Name = "LockButton"
LockButton.Parent = NexoFlopGUI
...
LockButton.Image = "rbxassetid://284402785"
...
Script.LookAt = function(Target)
    ...
    if RootPart and getgenv().NexoFlop['Features']['Look At'] then
        if Target and Target.Character and Target.Character:FindFirstChild(getgenv().NexoFlop['Target Part']) then
            local TargetPart = Target.Character[getgenv().NexoFlop['Target Part']]
            local TargetPos = TargetPart.Position
            local TargetVel = Script.CalculateVelocity(Target)
            local PredictedPos = TargetPos
            
            if getgenv().NexoFlop['Auto Prediction'] then
                local ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
                getgenv().NexoFlop['Prediction']['Amount'] = 0.1 + (ping / 2500) * 0.88
            end
            
            PredictedPos = TargetPos + (TargetVel * getgenv().NexoFlop['Prediction']['Amount'])
            
            if TargetVel.Y > 15 then
                PredictedPos = PredictedPos + Vector3.new(0, getgenv().NexoFlop['Offset']['Jump'], 0)
            end
...
Script.AutoAir = function(Target)
    if getgenv().NexoFlop['Features']['Auto Air'] and Target and Target.Character then
...
Script.OnToolActivated = function()
    if Locking and Target and Target.Character then
        local part = Target.Character:FindFirstChild(getgenv().NexoFlop['Target Part'])
        if part then
            local position = part.Position + (PlayerData[Target].Velocity * getgenv().NexoFlop['Prediction']['Amount'])
...
Script.HighlightTarget = function(target)
    if getgenv().NexoFlop['Features']['Highlight Target'] and target and target.Character then
        Highlight.Adornee = target.Character
        Highlight.FillColor = getgenv().NexoFlop['Highlight']['FillColor']
        Highlight.OutlineColor = getgenv().NexoFlop['Highlight']['OutlineColor']
        Highlight.FillTransparency = getgenv().NexoFlop['Highlight']['FillTransparency']
        Highlight.OutlineTransparency = getgenv().NexoFlop['Highlight']['OutlineTransparency']
...
Script.Strafe = function(target)
    if getgenv().NexoFlop['Strafe']['Enabled'] and target and target.Character then
        local character = LocalPlayer.Character
        if character and character:FindFirstChild("HumanoidRootPart") then
            local current_time = tick()
            local cframe = CFrame.new(target.Character.HumanoidRootPart.Position) * 
                           CFrame.Angles(0, 2 * math.pi * current_time * getgenv().NexoFlop['Strafe']['Speed'] % (2 * math.pi), 0) * 
                           CFrame.new(0, getgenv().NexoFlop['Strafe']['Height'], getgenv().NexoFlop['Strafe']['Distance'])
...
Script.CFrameWalk = function()
    if getgenv().NexoFlop['CFrame Walk']['Enabled'] then
        local character = LocalPlayer.Character
        if character and character:FindFirstChild("HumanoidRootPart") and character:FindFirstChild("Humanoid") then
            local moveDirection = character.Humanoid.MoveDirection
            if moveDirection.Magnitude > 0 then
                character.HumanoidRootPart.CFrame = character.HumanoidRootPart.CFrame + moveDirection * getgenv().NexoFlop['CFrame Walk']['Speed']
            end
        end
    end
end
...
RunService.Heartbeat:Connect(function()
    if Locking and Target then
        ...
        Script.HighlightTarget(Target)
        Script.Strafe(Target)
    else
        Script.HighlightTarget(nil)
    end
    Script.CFrameWalk()
end)
...
LockButton.MouseButton1Click:Connect(function()
    if Locking then
        ...
        Script.HighlightTarget(nil)
                Notification:Notify(
            {Title = "NexoFlop.lol", Description = "U hav Spared This motherfucker"},
            {OutlineColor = Color3.fromRGB(255, 0, 0), Time = 1.5, Type = "default"}
        )
    else
        Target = Script.FindClosestPlayer()
        if Target then
            Locking = true
            LockButton.Image = "rbxassetid://284402752"
            Notification:Notify(
                {
                    Title = "NexoFlop.lol",
                    Description = "Targetlock On this Nigga: " .. tostring(Target.Character.Humanoid.DisplayName)
                },
                {
...
do
    local Aiming = Pages["Aiming"]:Section({Name = "Main", Side = "Left"})

    Aiming:List({
        Name = "Target Part", 
        Flag = "TargetPart",
        Options = {"HumanoidRootPart", "Head", "UpperTorso", "LowerTorso", "RightUpperLeg", " LeftUpperLeg"},
        Default = getgenv().NexoFlop['Target Part'],
        Callback = function(Value)
            getgenv().NexoFlop['Target Part'] = Value
        end
    })
    ...
    Aiming:Textbox({
        Name = "Prediction",
        Flag = "Prediction",
        Default = tostring(getgenv().NexoFlop.Prediction.Amount),
        Callback = function(Value)
            getgenv().NexoFlop.Prediction.Amount = tonumber(Value) or getgenv().NexoFlop.Prediction.Amount
        end
    })
    ...
    Aiming:Textbox({
        Name = "Jump Offset",
        Flag = "JumpOffset",
        Default = tostring(getgenv().NexoFlop.Offset.Jump),
        Callback = function(Value)
            getgenv().NexoFlop.Offset.Jump = tonumber(Value) or getgenv().NexoFlop.Offset.Jump
        end
    })
    ...
    Aiming:Toggle({
        ...
        Flag = "AutoPrediction",
        Default = getgenv().NexoFlop['Auto Prediction'],
        Callback = function(Value)
            getgenv().NexoFlop['Auto Prediction'] = Value
        end
    })

    local Features = Pages["Aiming"]:Section({Name = "Features", Side = "Right"})

    Features:Toggle({
        Name = "Look At", 
        Flag = "LookAt",
        Default = getgenv().NexoFlop.Features['Look At'],
        Callback = function(Value)
            getgenv().NexoFlop.Features['Look At'] = Value
        end
    })
    ...
    Features:Toggle({
        Name = "Auto Air", 
        Flag = "AutoAir",
        Default = getgenv().NexoFlop.Features['Auto Air'],
        Callback = function(Value)
            getgenv().NexoFlop.Features['Auto Air'] = Value
        end
    })
    ...
    Features:Toggle({
        ...
        Default = getgenv().NexoFlop.Features['Highlight Target'],
        Callback = function(Value)
            getgenv().NexoFlop.Features['Highlight Target'] = Value
        end
    })

    local Highlight = Pages["Aiming"]:Section({Name = "Highlight", Side = "Right"})

    Highlight:Colorpicker({
        Name = "Fill Color",
        Default = getgenv().NexoFlop.Highlight.FillColor,
        Callback = function(Value)
            getgenv().NexoFlop.Highlight.FillColor = Value
        end
    })

    Highlight:Colorpicker({
        Name = "Outline Color",
        Default = getgenv().NexoFlop.Highlight.OutlineColor,
        Callback = function(Value)
            getgenv().NexoFlop.Highlight.OutlineColor = Value
        end
    })
    ...
    Highlight:Textbox({
        ...
        Default = tostring(getgenv().NexoFlop.Highlight.FillTransparency),
        Callback = function(Value)
            getgenv().NexoFlop.Highlight.FillTransparency = tonumber(Value) or getgenv().NexoFlop.Highlight.FillTransparency
        end
    })
    ...
    Highlight:Textbox({
        ...
        Default = tostring(getgenv().NexoFlop.Highlight.OutlineTransparency),
        Callback = function(Value)
            getgenv().NexoFlop.Highlight.OutlineTransparency = tonumber(Value) or getgenv().NexoFlop.Highlight.OutlineTransparency
        end
    })

    local Strafe = Pages["Aiming"]:Section({Name = "Strafe", Side = "Left"})

    Strafe:Toggle({
        Name = "Enabled", 
        Flag = "StrafeEnabled",
        Default = getgenv().NexoFlop.Strafe.Enabled,
        Callback = function(Value)
            getgenv().NexoFlop.Strafe.Enabled = Value
        end
    })
    ...
    Strafe:Textbox({
        Name = "Speed",
        Flag = "StrafeSpeed",
        Default = tostring(getgenv().NexoFlop.Strafe.Speed),
        Callback = function(Value)
            getgenv().NexoFlop.Strafe.Speed = tonumber(Value) or getgenv().NexoFlop.Strafe.Speed
        end
    })
    ...
    Strafe:Textbox({
        Name = "Distance",
        Flag = "StrafeDistance",
        Default = tostring(getgenv().NexoFlop.Strafe.Distance),
        Callback = function(Value)
            getgenv().NexoFlop.Strafe.Distance = tonumber(Value) or getgenv().NexoFlop.Strafe.Distance
        end
    })
    ...
    Strafe:Textbox({
        Name = "Height",
        Flag = "StrafeHeight",
        Default = tostring(getgenv().NexoFlop.Strafe.Height),
        Callback = function(Value)
            getgenv().NexoFlop.Strafe.Height = tonumber(Value) or getgenv().NexoFlop.Strafe.Height
        end
    })

    local CFrameWalk = Pages["Aiming"]:Section({Name = "CFrame Walk", Side = "Left"})

    CFrameWalk:Toggle({
        Name = "Enabled", 
        Flag = "CFrameWalkEnabled",
        Default = getgenv().NexoFlop['CFrame Walk'].Enabled,
        Callback = function(Value)
            getgenv().NexoFlop['CFrame Walk'].Enabled = Value
        end
    })
    ...
    CFrameWalk:Textbox({
        ...
        Default = tostring(getgenv().NexoFlop['CFrame Walk'].Speed),
        Callback = function(Value)
            getgenv().NexoFlop['CFrame Walk'].Speed = tonumber(Value) or getgenv().NexoFlop['CFrame Walk'].Speed
        end
    })
    ...
    Config:Keybind({Name = "Menu Key", Flag = "MenuKey", Ignore = true, UseKey = true, Default = Enum.KeyCode.E, Callback = function(State) Library.UIKey = State end})
...
Notifications:Notification("Loaded External Mob: " .. math.floor(tick() - Tick) .. "seconds", 3, Library.Accent, false)
